...
function pack_ (pkg, cb) {
  cache.add(pkg, null, false, function (er, data) {
    if (er) return cb(er)
    var outputConfig = npm.config.get('output')
      , fname

    if (!outputConfig) fname = path.resolve(data._id.replace(/@/g, "-") + ".tgz")
    else if (outputConfig === '-') fname = '/dev/stdout'
    else fname = outputConfig

    var cached = path.resolve( npm.cache
                             , data.name
                             , data.version
                             , "package.tgz" )
